...
- `SpellEffectTintPatch` restores original particle and material colors on pooled projectile/FX GameObjects before applying a tint, and also when no tint is active, preventing pooled Magic Missile views from retaining stale custom missile tints.
- Ray-style projectile tinting is handled globally by `SpellEffectTintPatch` for all registered projectile tints, not per spell.
- The shared ray tint path recolors `LineRenderer` and `RayView` origin gradients, `TrailRenderer`, particle color-over-lifetime/color-by-speed/trails, `ParticleSystemRenderer.trailMaterial`, lights, and shader color/vector properties while preserving alpha.
...
- Cloned Magic Missile spells set `AbilityDeliverProjectile.DelayBetweenProjectiles = 0.08f` to stagger repeated missile visuals through `SpellInstaller`'s projectile visual path; do not apply this delay broadly to non-Magic-Missile projectile spells.
...
- Shadowborn Umbral Ray clones `RayOfFrost00` projectile view (`d6c9daec1256561408a7a72a6979359e`) and Enervation cast/hit/miss settings, using speed `18f` so hit FX trigger sooner.
