...
- WotR `CompanionInParty` is unreliable for the Shield Maze Billy runtime stand-in and may count it as in-party before actual recruitment; dialog recruitment gating should use the Billy recruited flag or a custom condition instead.
...
- Billy's Shield Maze `RecruitData` uses `MustBeInParty = true` because WotR Recruit removes newly recruited companions and opens the party picker when `Party.Count` exceeds six; if the recruit is not marked required, Billy is not reserved in the picker and can fail to join parties with pets or extra members.
- Shield Maze pre-recruit Billy must use a separate stand-in BlueprintUnit GUID from the real companion; WotR Recruit checks `Player.AllCharacters` by companion blueprint before recruiting via `NPCUnit`, and a stand-in using the real GUID causes a double-recruit log.
- Existing saves may contain stale real-blueprint stand-ins, so recruitment/placement cleanup removes non-roster Billy units that lack `UnitPartCompanion` state.
- Billy dialogue cues leave `DialogSpeaker.m_Blueprint` null to use the dialog default/current speaker while optionally keeping Billy in `m_SpeakerPortrait`; hard-binding to the real companion blueprint makes pre-recruit Shield Maze dialog fail because that speaker does not yet exist.
...
- Act 1 reward stage places Irori Neophyte's Armor, a +1 breastplate with the retained bow attack/damage bonus, in the Tower of Estrod basement box as the cult cache reward.
...
- Act 2 uses two mandatory main-path areas in fixed order. The Vescavor Queen loot at Leper's Smile contains the +2 Acolyte's Longbow of Discipline, which replaces the +1 bow and advances the quest to Lost Chapel. A static library cupboard in the Lost Chapel contains +2 Irori Acolyte's Armor, which replaces the +1 armor and advances the quest toward the Ivory Sanctum.
- Act 3 uses the Ivory Sanctum and Midnight Fane in fixed main-quest order. Xanthir's notes contain +3 Irori Adept's Armor, which replaces the +2 armor and advances the quest to the Midnight Fane. Static balor-corpse loot in the Midnight Fane contains the +3 Adept's Longbow of Discipline, which replaces the +2 bow and starts the Act 4 Abyss lead.
- Equipment rewards use the tested item-acquisition handler pattern. Each trigger completes the active objective, removes one prior-tier item through `Player.Inventory.Remove(..., allowRemoveEquipped: true)`, starts the next objective, and opens a one-cue Billy dialogue when acquired directly from loot; area-load checks provide silent backfill.
- The upgraded armors use WotR's official positive- and negative-channeling bonus features. The negative feature matches `ChannelNegativeHeal`, whose effect is `HealMassUndead`, so the added dice apply when Billy heals undead allies with negative energy as well as when his channels deal damage or heal living allies.
- Act 2 and Act 3 dialogue cues use stable localization keys and the Wwise event names `Play_CMP_Billy_Dialog_Act2BowCue`, `Play_CMP_Billy_Dialog_Act2ArmorCue`, `Play_CMP_Billy_Dialog_Act3ArmorCue`, and `Play_CMP_Billy_Dialog_Act3BowCue` for the next audio-generation pass.
...
- Planned bow progression replaces the prior item instance at each tier: the original +1 Neophyte's Longbow of Discipline deals an additional 1d6 force damage to chaotic targets; the Act 2 +2 Acolyte's Longbow of Discipline deals 1d8; the second Act 3 reward, the +3 Adept's Longbow of Discipline, deals 1d10; the future +4 Prior's Longbow of Discipline deals 1d12; and the future +5 Archon's Longbow of Discipline deals 2d10.
- Billy's quest is planned to have at least two steps in each Act from Act 2 through Act 5, allowing both his bow and armor to advance by one tier during each Act.
- Planned armor progression also replaces the prior item instance at each tier: the original +1 Irori Neophyte's Armor, +2 Irori Acolyte's Armor, +3 Irori Adept's Armor, +4 Irori Prior's Armor, and +5 Irori Archon's Armor. Each armor's enhancement bonus and its bonus on attack and damage rolls with bows match its tier. The Act 2 +2 armor adds 1d6, and the Act 3 +3 armor adds 2d6, to both the healing and damage of Billy's positive and negative cleric Channel Energy abilities while equipped.
